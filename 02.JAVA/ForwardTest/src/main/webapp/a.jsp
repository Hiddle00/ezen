...
<body>
	...
	<%--
		EL(Expression Language, 표현식) <%= %>대체하여 사용
		${객체}형태로 사용
	...
	<!--
		el을 이용한 객체접근
		session객체에도 user키(sung)가 있고, reqeust객체에도 user키(hong)가 있다.
		request의 우선순위가 높기 때문에 위에 작성한 ${user}는
		request.getAttribute("user")랑 동일한 코드
		
		여러개의 객체(reqeust, session)에 동일한 키에 데이터가 들어가있다면
		명확히 sessionScope나 requestScope를 작성해야한다.
	-->
